...
class RoundedImage: NSImage {
    ...
    internal func addRoundedRectToPath(context: CGContext, rect: CGRect, ovalWidth: CGFloat, ovalHeight: CGFloat) {
    ...
    func withRoundCorners(radius: Int) -> NSImage {
        ...
        addRoundedRectToPath(context: context!, rect: rect, ovalWidth: CGFloat(radius), ovalHeight: CGFloat(radius))
        context?.closePath()
        context?.clip()
